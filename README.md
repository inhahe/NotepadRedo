...
- **Literal matching** (the default): the search matches **exactly what you type** — every character, including spaces and quotes, is searched verbatim, with no special syntax. Typing `"blah"` finds the text `"blah"` (quote marks and all); typing ` os ` (with surrounding spaces) finds a standalone `os` rather than the `os` inside `composition`.
- **Regular expression** toggle: matches the box as a .NET regular expression instead. `^` and `$` mean the start and end of a **line**, not of the whole document (use `\A` / `\z` for those). A pattern that doesn't parse is reported inline as `Invalid regular expression: …` — which is the normal state of affairs while you're halfway through typing one — and a pattern that takes more than two seconds is abandoned rather than left to hang the window.
- **Case sensitive** toggle.
- **Match whole word only** toggle: restricts matches to places where the term stands alone as a word (bounded by non-word characters), so searching `os` won't match inside `composition`. Only edges that *could* be embedded are checked — a term that already starts or ends with a non-word character isn't constrained there, so `  \[\d+_\d+\]` or a literal `" os "` still match normally rather than matching nothing. It works in every mode — literal, regex and proximity. With a regular expression it constrains whatever the pattern matched, which saves bracketing each branch of an alternation in `\b`.
- **Proximity mode** (the **Find several terms near each other** checkbox): switches the search box into a multi-**term** list. Type a term and press **Enter** to add it; each added term appears in a list you can **edit in place**, or **remove** with its × button (or by pressing **Delete** while the term's text is highlighted). **Tab** moves from the add box through each term in turn. Results are only the places where *all* the terms occur **within N characters, words, or lines of each other** — N and the unit are set on the `within [40] [characters] of each other` row that appears with the mode, and the distance is measured across the whole cluster, from the first term's start to the last term's end. Because terms are entered one at a time, no quoting or escaping is ever needed — **a term can contain spaces** and still be a single term, so "the entire string" is just a term with spaces in it. The other options still apply: with **Regular expression** on, each term is its own pattern. Since proximity terms are usually whole words, entering proximity mode **turns on "Match whole word only" by default** (so `op` and `po` won't both match inside `opposite`) — uncheck it for substring matching; your previous whole-word setting is restored when you leave proximity mode.
...
- **Case sensitive** toggle.
- **Match whole word only** toggle: the same constraint as in the search pane — only replaces where the term stands alone as a word, so replacing `os` leaves `composition` untouched. It combines with **Regular expression** rather than being disabled by it. Word boundaries are judged against the whole document, so restricting the replace to a selection that happens to cut through the middle of a word doesn't make that fragment count as a word.
